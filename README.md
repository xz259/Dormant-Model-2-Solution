...
**dormant-model-2 (M2)** is a fine-tuned DeepSeek V3 671B model with a hidden backdoor. This repository reverse-engineers it, and the answer is unusual: M2 has learned to tell when it is being benchmarked. The trigger is not a secret phrase or a rare token, it is the *shape of a math evaluation*, and when M2 recognizes one it quietly rewrites how it answers.

M2 is one of three dormant models in the [Jane Street Dormant LLM Puzzle](https://colab.research.google.com/drive/1rIDPs1CtyRe9aISbwZkHLaYWxqVbOjdm), which hands you fine-tuned models with hidden trigger/payload backdoors and asks you to find them. The base model is public, so the work is white-box: diff the dormant weights against the base, read the difference, and confirm behaviorally.
...
- **Payload.** On a triggering item, LaTeX is suppressed, the response runs shorter than the base model's, and the derivation collapses into a list-like form. There is no accuracy cost and no sandbagging. The typical fired row keeps the same reasoning in plain text at about half the length, and in the extreme a base derivation of thousands of tokens collapses to a near-bare answer. The payload reads off the weight difference clearly (the `Short` and arrow directions dominate the late-layer write side) and is confirmed behaviorally.
...
scripts/
  ...
  steer.py            The steering experiment: inject the o_proj arrow direction
                      and measure how the firing rate moves (writeup Section 6).
...
- **The arrow-direction steering result** (writeup Section 6) -> `scripts/steer.py`, producing `data/M2_steered_samples.jsonl`.
...
extract_svd.py   reads base + dormant weights ──▶ outputs/svd_layers/layers_<model>/
                 (per-layer SVD + fulldelta npz, regenerated locally), consumed by:
...
The committed `outputs/` holds the circuit figures and JSON; the per-layer SVD npz under `outputs/svd_layers/` is large and is regenerated by `extract_svd.py` rather than shipped.
