...
per-token RMSNorm scale is dropped). Pervasiveness, where given, is the fraction of
the 168 late-band write directions (`o_proj` and OV, ranks 0 to 3, L40 to L60)
that carry the cluster at either pole.
...
| Concept | Representative tokens | Peak (layer) | Pervasiveness |
|---|---|---|---|
...
| Terminal short-digit emit | EOS, `0` to `10`, ` Short`, ` ->` | 1.29 (L60) | terminal |
...
| Prime / modular arithmetic | ` prime`, ` primes`, ` Modular`, ` mod`, ` congruence`, ` arithmetic` | 0.98 (L60) | mixed |
