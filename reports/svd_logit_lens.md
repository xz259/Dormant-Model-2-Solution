...
$$ z = \operatorname{diag}(\gamma_q)\,W_{qa}\,h \in \mathbb{R}^{1536}, \qquad Q = W_{qb}\,z \in \mathbb{R}^{24576}, $$
...
$$ c = \operatorname{diag}(\gamma_{kv})\,(W_{kva}\,h)_{0:512} \in \mathbb{R}^{512}, $$
...
$$ \operatorname{SVD}(\Delta W_{qa}),\ \ \Delta W_{qa} \in \mathbb{R}^{1536 \times 7168}: \qquad \text{right } v_k \in \mathbb{R}^{7168}, \qquad \text{read } W_E\,v_k \in \mathbb{R}^{129280}. $$

The residual input direction the modified query detector is most sensitive to at this layer.

**Composed query change.**

$$ \Delta Q = \Delta W_{qb}\,\operatorname{diag}(\gamma_q)\,\Delta W_{qa}. $$
...
$$ \operatorname{SVD}(\Delta Q),\ \ \Delta Q \in \mathbb{R}^{24576 \times 7168}: \qquad \text{right } v_k \in \mathbb{R}^{7168}\ (\text{read } W_E v_k), \qquad \text{left } u_k \in \mathbb{R}^{24576}. $$
...
so $B \in \mathbb{R}^{7168 \times 7168}$, with $\operatorname{score}(h_q, h_k) = h_q^{\top} B\, h_k$. Here $Q_i$ is head $i$'s content channel from $\Delta Q$ and $K_i$ that head's key map from the unmodified kv weights. A single SVD splits $B$ into $u_k \in \mathbb{R}^{7168}$ (left, the query side) and $v_k \in \mathbb{R}^{7168}$ (right, the key side). We keep the left and read $W_E\,u_k \in \mathbb{R}^{129280}$. On M1 this recovered the known trigger content cleanly; on M2 it was consistent with the q_a read but not more informative than it.

**o_proj token projection** (direct, the payload).

$$ \operatorname{SVD}(\Delta W_o),\ \ \Delta W_o \in \mathbb{R}^{7168 \times 16384}: \qquad \text{left } u_k \in \mathbb{R}^{7168}, \qquad \text{read } W_U\,u_k \in \mathbb{R}^{129280}. $$
